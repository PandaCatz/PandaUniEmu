...
| Generated PPU shell and background-fetch expectations | Committed as Rust unit/integration tests | Project-owned cases derived from factual NES register, [scrolling](https://www.nesdev.org/wiki/PPU_scrolling), and [rendering](https://www.nesdev.org/wiki/PPU_rendering) descriptions; no external ROM, result data, or local-course code is copied | Focused coverage for CPU-port behavior, memory routing, two-dot nametable/attribute/pattern phases, all scroll wrap branches, prefetch/dummy reads, shifter reloads, rendering-time `$2007` increment without ordinary VRAM transfer, and odd-frame dummy completion; these specification-derived numeric self-tests are not an independent hardware/test-ROM oracle and do not cover sprites, pixels, PPUMASK propagation, or status races |
