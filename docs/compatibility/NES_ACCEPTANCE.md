...
| Independent CPU trace | Operator-supplied `nestest` trace matches PC, A, X, Y, P, SP, and cumulative cycles through the final end-state row under `NESTEST_PROCEDURE.md` | Bounded parser, bus, generated comparison, operator-path CLI, and fixture identity review pass; no operator pair has been run |
