...
| Generated PPU shell expectations | Committed as Rust unit/integration tests | Project-owned cases derived from factual NES register/address descriptions in the linked NESdev references; no external ROM, result data, or local-course code is copied | Focused coverage for CPU-port mirroring, register side effects, `v/t/fine-X/write-toggle`, PPUDATA buffering, CHR ROM/RAM, nametable/palette mirroring, OAM ports, and logical VBlank-driven NMI; these self-tests are not an independent PPU hardware-equivalence oracle |
