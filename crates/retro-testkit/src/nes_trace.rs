...
mod tests {
    ...
    fn cleanroom_nrom_cases_match_pinned_images_and_architectural_traces() {
        ...
        for case in crate::cleanroom_nrom::CASES {
            ...
            assert_eq!(
                sha256_hex(case.trace.as_bytes()),
                case.trace_sha256,
                "{} trace",
                case.name
            );

            let summary = compare_nrom_trace_bytes(&image, case.trace.as_bytes())
                .unwrap_or_else(|error| panic!("{} clean-room trace failed: {error}", case.name));
            assert_eq!(summary.rows_matched, case.rows, "{} rows", case.name);
            assert_eq!(
                summary.transitions_verified, case.transitions,
                "{} transitions",
                case.name
            );
            assert_eq!(
                summary.final_state.total_cycles, case.final_cycles,
                "{} cycles",
                case.name
            );
            assert_eq!(summary.final_state.pc, 0xc102, "{} PC", case.name);
            assert_eq!(summary.final_state.a, 0x5a, "{} A", case.name);
            assert_eq!(summary.final_state.x, 0x01, "{} X", case.name);
            assert_eq!(summary.final_state.y, 0x00, "{} Y", case.name);
            assert_eq!(summary.final_state.sp, 0xfd, "{} SP", case.name);
        }
    }
